...
the csspin-python project.

The csspin-python repository is hosted on a private GitLab instance, while the
repository itself is mirrored to
[GitHub](https://github.com/cslab/csspin-python) for public visibility.
...
Please follow the guidelines below when contributing to the csspin-python
project.
...
repository](https://github.com/cslab/csspin-python/issues) for reporting issues
...
3. `git clone <your-fork-or-repo-url> csspin-python && cd csspin-python && pre-commit install`
4. `uv venv venv && source venv/bin/activate && uv pip install -r
requirements-dev.txt` (to set up the development environment)
...
> This section is only relevant for maintainers of the csspin-python project.

The version scheme used is major.minor.patch following semver.org.

Releases are created by csspin-python maintainers within the GitLab instance hosting
this repository.
...
1. Create the release notes by opening a new merge request at the csspin-python
